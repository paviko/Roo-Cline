...
describe('enhancePrompt', () => {
    ...
    it('should throw error for non-OpenRouter providers', async () => {
        ...
        await expect(enhancePrompt(nonOpenRouterConfig, 'test', Uri.file('/'))).rejects.toThrow('Prompt enhancement is only available with OpenRouter')
    })
    ...
    it('should enhance a valid prompt', async () => {
        ...
        const result = await enhancePrompt(mockApiConfig, inputPrompt, Uri.file('/'))
    ...
    it('should throw error when no prompt text is provided', async () => {
        await expect(enhancePrompt(mockApiConfig, '', Uri.file('/'))).rejects.toThrow('No prompt text provided')
    ...
    it('should pass through API errors', async () => {
        ...
        await expect(enhancePrompt(mockApiConfig, inputPrompt, Uri.file('/'))).rejects.toBe('API error')
    })
    ...
    it('should pass the correct prompt format to the API', async () => {
        ...
        await enhancePrompt(mockApiConfig, inputPrompt, Uri.file('/'))
