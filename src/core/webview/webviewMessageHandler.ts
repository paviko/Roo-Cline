...
export const webviewMessageHandler = async (provider: ClineProvider, message: WebviewMessage) => {
	...
	switch (message.type) {
		...
		case "requestsPerMinuteLimit":
			await updateGlobalState("requestsPerMinuteLimit", message.requestsPerMinuteLimit)
			await provider.postStateToWebview()
			break
...
const generateSystemPrompt = async (provider: ClineProvider, message: WebviewMessage) => {
	...
	try {
		const tempApiHandler = buildApiHandler(apiConfiguration, provider.context.extensionUri)
