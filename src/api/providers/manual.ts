...
export class ManualHandler implements ApiHandler {
	...
	private getRequest(messages: Anthropic.Messages.MessageParam[]): string {
		return messages.length > 0
			...
			? (messages[messages.length - 1].content as Array<Anthropic.Messages.TextBlockParam | Anthropic.Messages.ImageBlockParam | Anthropic.Messages.ToolUseBlockParam | Anthropic.Messages.ToolResultBlockParam>)
				.filter((block) => block.type === 'text')
