...
const getCommandsMap = ({ context, outputChannel, provider }: RegisterCommandOptions) => {
	return {
		"roo-cline.addToContext": async (uri: vscode.Uri) => {
		...
		"roo-cline.addAllOpenToContext": async () => {
			const visibleProvider = ClineProvider.getVisibleInstance()
			if (!visibleProvider) {
				return
			}

			const openFilePaths = vscode.window.tabGroups.all.flatMap(({ tabs }) =>
				tabs
					.map((tab) => {
		...
		"roo-cline.addActiveToContext": async () => {
			const visibleProvider = ClineProvider.getVisibleInstance()
			if (!visibleProvider) {
				return
			}
		...
		"roo-cline.kebabButtonClicked": async () => {
			const visibleProvider = ClineProvider.getVisibleInstance()
			if (!visibleProvider) {
				return
			}
