...
export const OpenAICompatible = ({
    ...
    // Only propagate when the processed headers actually differ from the current configuration
    // Mark as a non-user action when this is just an initialization sync to avoid dirty state on open
    useEffect(() => {
        const timer = setTimeout(() => {
            const headerObject = convertHeadersToObject(customHeaders)
            const current = apiConfiguration?.openAiHeaders || {}
            if (JSON.stringify(current) !== JSON.stringify(headerObject)) {
                setApiConfigurationField("openAiHeaders", headerObject, false)
            }
        }, 300)

        return () => clearTimeout(timer)
    }, [customHeaders, setApiConfigurationField, apiConfiguration?.openAiHeaders])
